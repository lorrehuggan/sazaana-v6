...
export default function Authentication() {
  ...
  return (
    <>
      {!session ? (
        <button className={style.signIn} onClick={() => signIn('spotify')}>
          <svg
            width="256px"
            height="256px"
            viewBox="0 0 256 256"
        ...
        <Popover.Root>
          <Popover.Trigger>
            <Avatar.Root className={style.avatar} onClick={() => signOut()}>
              <Avatar.Image
                src={session.user.image}
                className={style.avatar__image}
              />
              <Avatar.Fallback className={style.avatar__fallback}>
